...
exports.findAll = (req, res) => {
  ...
  Grid.find(condition)
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || "Some error occurred while retrieving grids.",
      });
    });
};
...
exports.findOne = (req, res) => {
  ...
  Grid.findById(id)
    ...
    .catch((err) => {
      res.status(500).send({ message: "Error retrieving Grid with id=" + id });
    });
};
...
exports.deleteAll = (req, res) => {
  Grid.deleteMany({})
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || "Some error occurred while removing all grids.",
      });
    });
};
