...
exports.create = (req, res) => {
  ...
  Project.findOneAndUpdate({_id: root}, {$push: { tasks: req.body}}, { useFindAndModify: true })
    .then(data => {
      if (!data) {
        res.status(404).send({
          message: `Cannot create Task. Maybe Project was not found!`
        });
      } else {
        try {
    ...
    Project.findOneAndUpdate(
      ...
      .then(data => {
        if(req.body.isAdmin) {
          ...
          //     if(!data) {
          //         console.log(`Cannot update User value with id=${resource._id} in task(${taskId}) for Project with id=${id}`)
...
exports.delete = (req, res) => {
  ...
  Project.findOneAndUpdate({_id: id }, { $pull: {tasks:{_id : taskId}}},{ useFindAndModify: false })
    .then(data => {
      if (!data) {
        res.status(404).send({
